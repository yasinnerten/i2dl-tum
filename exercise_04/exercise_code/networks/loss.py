...
class BCE(Loss):
    ...
    def forward(self, y_out, y_truth, individual_losses=False):
        ...
        result = None
        eps = 1e-12
        probs = np.clip(y_out, eps, 1 - eps)
        result = -(y_truth * np.log(probs) + (1 - y_truth) * np.log(1 - probs))
    ...
    def backward(self, y_out, y_truth):
        ...
        eps = 1e-12
        probs = np.clip(y_out, eps, 1 - eps)
        gradient = -(y_truth / probs) + (1 - y_truth) / (1 - probs)
        gradient = gradient / len(y_truth)
        return gradient
