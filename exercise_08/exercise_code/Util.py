...
def checkParams(model):
    ...
    if n_params > PARAM_LIMIT:
        print(
            "# Parameters: Your model has {} mio. params but must have less than 5 mio. params. Simplify your model before submitting it. You won't need that many params :)".format(
            bcolors.colorize(color, normalized)))
        return False
    print("# Parameters: Your model has {} mio. params.".format(bcolors.colorize(color, normalized)))
    return True
